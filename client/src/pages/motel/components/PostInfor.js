...
class PostInfor extends React.Component {
  constructor(props) {
        ...
        render: () =>
          <div style={{}}>
            <Tooltip title="Xem chi tiết" placement="left">
              <img style={{ width: "100px", height: "80px", cursor: "pointer" }} onClick={() => {
        ...
        render: (text, record) =>
          this.state.dataSource.length >= 1 ? (
            <Popconfirm title="Sure to delete?" onConfirm={() => this.handleDelete(record.key)}>
              <Button style={{ background: "#1890ff", borderColor: "#1890ff", textShadow: '-1px 0 rgba(0, 0, 0, 0.12)', color: "white" }}>Delete</Button>
            </Popconfirm>
          ) : null,
      },
      {
    ...
    this.state = {
      dataSource: [// mảng chứa thông tin của các nhà trọ
         {
          key: '0',
  ...
  handleAdd = () => {
    ...
    const newData = {//thông tin của nhà trọ khi được thêm vào
      key: count,
      title: `Edward King `,
  ...
  render() {
    ...
    return (
      <div>
        <ModalMenu handleAdds={() => this.handleAdd()} name="Thêm bài đăng" button="Add a motel"></ModalMenu>
